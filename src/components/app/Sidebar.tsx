...
export function Sidebar() {
  ...
  const creditsPercent = Math.round((creditsUsed / creditsLimit) * 100)
        ...
        {!sidebarCollapsed && (
          <>
            <div className="flex-1 min-w-0">
              <div className="text-sm font-bold text-[#2D1C12] truncate leading-tight">
                {user?.fullName ?? user?.emailAddresses?.[0]?.emailAddress ?? "User"}
              </div>
              <span className={cn("text-[9px] font-black px-1.5 py-0.5 rounded-full inline-block mt-0.5", PLAN_COLORS["free"])}>
                FREE
              </span>
            </div>
            <button
